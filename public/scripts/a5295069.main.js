...
(function() {
  define('core/session/providers/cookies',["core/api/auth"], function(ApiAuth) {
    var ProviderCookie;
...
(function() {
  define('core/session/providers/local-storage',["core/api/auth"], function(ApiAuth) {
...
(function() {
  define('core/session/providers/memory',["core/api/auth"], function(ApiAuth) {
...
(function() {
  define('core/session/providers',['core/session/providers/cookies', 'core/session/providers/local-storage', 'core/session/providers/memory'], function(ProviderCookie, ProviderLocalStorage, ProviderMemory) {
    return {
...
(function() {
  define('core/api/exception',[], function() {
    var ApiException;
...
define("cryptojs.hmac.sha1", function(){});
...
(function() {
  define('core/api/request',['core/api/exception', 'cryptojs.hmac.sha1', 'cryptojs.sha256'], function(ApiException) {
    var ApiRequest;
...
(function() {
  define('core/api/sync',["backbone", "core/api"], function(Api) {
    var methodMapper;
    ...
    return Backbone.sync = function(method, model, options) {
      ...
      data = {};
      secure = true;
      return Api[methodMapper(method)](url, data, secure).then(options.success)["catch"](options.error);
    };
  });
...
(function() {
  define('core/api',['when', 'core/api/exception', 'core/api/request', 'core/api/auth', 'config', 'core/api/sync'], function(When, ApiException, ApiRequest, ApiAuth, app_config) {
    var Api, app,
      _this = this;
    app = null;
    Api = (function() {
    ...
    _.each({
      ...
      var _this = this;
      return Api[helper] = function(url, data, secure) {
        var config, request;
        if (data == null) {
          data = null;
        }
        ...
        if (secure) {
          ...
          config.secure = true;
          config.auth = app.session.auth;
        }
        request = new ApiRequest(config, secure);
        return Api.proceed(request);
      };
    });
    return {
      inject: function(_app) {
        return app = _app;
      },
      Api: Api,
...
define("cryptojs.pbkdf2", function(){});

(function() {
  var __indexOf = [].indexOf || function(item) { for (var i = 0, l = this.length; i < l; i++) { if (i in this && this[i] === item) return i; } return -1; };

  define('core/session',['app', 'when', "core/session/providers", "core/api", 'cryptojs.pbkdf2', 'cryptojs.hmac.sha1', 'cryptojs.sha256'], function(app, When, SessionProviders, Api) {
    ...
    Session = (function() {
      ...
      Session.prototype.initialize = function(auth, user, provider) {
        this.auth = auth;
        this.user = user;
      ...
      */
      ...
      Session.prototype.logout = function() {
        var defer;
        defer = When.defer();
        app.API.update('logout', function(err, response) {
    ...
    Session.getToken = function(login, password, salt1, salt2) {
      var hmac, pkdf, token;
      pkdf = CryptoJS.PBKDF2(password, salt1, {
        keySize: 256 / 32,
        iterations: 1000,
        hasher: CryptoJS.algo.SHA256
      });
      hmac = CryptoJS.algo.HMAC.create(CryptoJS.algo.SHA256, salt2.concat(pkdf));
      hmac.update(login);
      token = hmac.finalize();
      return token;
    };
    Session.restore = function() {
      ...
      if (!found) {
        ...
        if (!auth) {
          ...
          Api.Api.proceed(request).then((function(response) {
            return defer.resolver.resolve(new Session(auth, {}, provider));
    ...
    Session.login = function(login, password, trusted_browser) {
      ...
      Api.Api.update('/login', {
        ...
        if ((response.salt1 != null) && (response.salt2 != null)) {
          ...
          auth = new Api.Auth(login, Session.getToken(login, password, CryptoJS.enc.Hex.parse(response.salt1), CryptoJS.enc.Hex.parse(response.salt2)));
          request = new Api.Request({
            url: "/api/user/me",
            method: "GET",
            auth: auth
          });
          check_session = Api.Api.proceed(request);
...
(function() {
  define('core/models/user',['backbone'], function() {
...
(function() {
  define('core/models/organization',['backbone'], function() {
    return Backbone.Model.extend({
      url: "/organizations"
    });
  });
...
(function() {
  define('core/models',['core/models/user', 'core/models/organization'], function(User, Organization) {
    return {
      "User": User,
...
(function() {
  ...
  define('core/router',['when', 'when-sequence', 'loglevel', 'backbone'], function(When, When_sequence, log) {
    /*
       Yoda Router
    */
      ...
      route: function(routes, name, callback, configurators) {
        ...
        return _.each(routes, function(route) {
          return Backbone.Router.prototype.route.call(_this, route, name, function() {
            ...
            log.info("route " + name + " with configurators =", configurators);
            configurators = _.each(_.clone(configurators), function(configurator_config, configurator_name) {
              ...
              if (configurator_config !== false) {
                configurator_scope = function() {
                  var defer;
                  defer = When.defer();
                  _this._configurators[configurator_name].call(_this, _this._app, route, configurator_config, defer.resolver.resolve, defer.resolver.reject);
                  return defer.promise;
                };
            ...
            return When_sequence(_configurators).done(function() {
              ...
              return log.info('neh!', e);
            });
          });
        });
      },
      boot: function() {
        log.info('Router.boot !');
...
(function() {
  define('core/core',['core/session', 'core/models', 'core/router', 'core/api'], function(Session, Models, Router, Api) {
    return {
      Router: Router,
      Session: Session,
      Api: Api,
...
(function() {
  define('app',['core/core', 'loglevel', "when", 'backbone.marionette'], function(Squareteam, log, When) {
    var app, configure,
      _this = this;
    log.enableAll();
    window.Core = Squareteam;
    /*
      Squareteam app is a Singleton across all requirejs modules
    */

    app = new Backbone.Marionette.Application();
    Squareteam.inject(app);
    configure = function(config) {
      var defer;
      defer = When.defer();
      app.router = new Squareteam.Router(config.router, app);
      if ((config.session != null) && config.session.autoload) {
        Session.restore().done(function(session) {
          app.session = session;
          return defer.resolver.resolve(app);
        }, function(e) {
          defer.resolver.reject();
          return log.warn(e);
        });
      } else {
        app.session = new Squareteam.Session.Anonymous();
        defer.resolver.resolve(app);
      }
      return defer.promise;
    };
    app.addInitializer(function(options) {
      log.info('bootstrap Squareteam');
      app.router.configurator('acl', function(app, route, config, resolve, reject) {
        if ((config.anonymous != null) && !app.session.isAuthenticated()) {
          reject({
      ...
      app.router.route('home', 'home', function() {
        return log.info('home');
      });
      app.router.route(['knowledge', 'knowledge/*path'], 'knowledge', function(path) {
        return require(['apps/knowledge/default']);
      });
      app.router.route(['missions', 'missions/*path'], 'missions', function(path) {
        log.info("mission ! " + path);
        return require(['apps/missions/default']);
      });
      app.router.route('login', 'login', function() {
        return log.info('login');
      }, {
        acl: false
      });
      app.router.route('about', 'about', function() {
        return log.info('about');
      }, {
        acl: false
      });
...
require.config({
    paths: {
        ...
        'cryptojs.hmac.sha1': '../bower_components/cryptojslib/rollups/hmac-sha1',
        'cryptojs.sha256': '../bower_components/cryptojslib/rollups/sha256'
    },
    shim: {
        jquery: {
            exports: 'jQuery'
...
require(['app', 'underscore', 'backbone', 'jquery'], function (app) {
    'use strict';
    ...
    app.configure({
        ...
        window.Squareteam = app; // debug
    }, function(e) {
        console.error('Fails to configure Squareteam', e);
    });
});
